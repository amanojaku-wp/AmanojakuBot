...
本文件是给在 VS Code / WebStorm 中接续开发的编码助手阅读的项目事实和需求摘要。具体实现以代码和测试为准；不能假定助手能够看到此前的 ChatGPT 对话。
...
- SQLite + `better-sqlite3` + 裸 SQL，不使用 ORM。YAML + Zod 配置，Pino 日志，Vitest 测试。密钥放环境变量，不写进 Git。
...
讨论页请求只接受条目（命名空间 0）和草稿；页面不存在不计额度；重定向要跟踪最终目标再检验命名空间和存在性。每位非主人编者按 UTC 自然日最多查询 10 篇有效条目；请求超额只处理前十个有效目标，解释无效目标及额度计算。首次请求后 30 日内有一次复查机会；复查不消耗次日及之后的新请求额度；复查机会用完后再请求视为新请求。只对错别字、文法、可能的事实错误、逻辑问题、疑似 AI 内容给出审慎意见。
